#ifndef SIMPROP_UNITSLITERALS_H
#define SIMPROP_UNITSLITERALS_H

#ifndef SIMPROP_UNITSDISABLE
...
#define BASELITERAL(_QUANTITY, _STRING)                                          \
  constexpr _QUANTITY operator"" _STRING(long double x) { return _QUANTITY(x); } \
  constexpr _QUANTITY operator"" _STRING(unsigned long long int x) { return _QUANTITY(static_cast<double>(x)); }

#define LITERAL(_QUANTITY, _STRING, _UNIT)                                    \
  constexpr _QUANTITY operator"" _STRING(long double x) { return x * _UNIT; } \
  constexpr _QUANTITY operator"" _STRING(unsigned long long int x) { return static_cast<double>(x) * _UNIT; }
