...
void plot_rates() {
  ...
  // const auto pppebl = interactions::PhotoPionProduction(sigma, ebl, rng);
...
void plot_pion_energies() {
  ...
  RandomNumberGenerator rng = utils::RNG<double>(1234);
  const auto pppcmb = std::make_shared<interactions::PhotoPionProduction>(sigma, cmb);
  utils::OutputFile out("test_pionenergy_sample2.txt");
  for (size_t i = 0; i < 100000; ++i) {
    if (i % 100 == 0) std::cout << i << "\n";
    out << i << "\t";
    out << pppcmb->samplePionEnergy(1e20 * SI::eV, 0., rng) / SI::eV << "\t";
    out << pppcmb->samplePionEnergy(1e21 * SI::eV, 0., rng) / SI::eV << "\t";
    out << pppcmb->samplePionEnergy(1e22 * SI::eV, 0., rng) / SI::eV << "\t";
    out << "\n";
  }
}
