...
void plot_rates() {
  ...
  const auto pppebl = std::make_shared<interactions::PhotoPionProduction>(ebl);
  {
    ...
    for (auto Gamma : Gammas) {
      ...
      out << pppebl->computeRateComoving(Gamma, 1.0, sigma) / SI::cLight / units << "\t";
      out << pppebl->computeRateComoving(Gamma, 2.0, sigma) / SI::cLight / units << "\t";
      out << "\n";
    }
  }
  {
...
void plot_sampled_s() {
  ...
  for (size_t i = 0; i < 300000; ++i) {
    ...
    out << i << "\t";
    out << interactions::sampleS(rng(), 4. * SI::GeV2, xs) / SI::GeV2 << "\t";
    out << interactions::sampleS(rng(), 40. * SI::GeV2, xs) / SI::GeV2 << "\t";
    out << "\n";
  }
}
...
void plot_sampled_epsilon() {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 100000; ++i) {
      ...
      out << i << "\t";
      const double E = 1e19 * SI::eV;
      out << interactions::sampleEps(rng(), E, 0., xs, cmb) / SI::eV << "\t";
      out << interactions::sampleEps(rng(), E, 0., xs, ebl) / SI::eV << "\t";
      out << interactions::sampleEps(rng(), E, 1., xs, cmb) / SI::eV << "\t";
      out << interactions::sampleEps(rng(), E, 1., xs, ebl) / SI::eV << "\t";
      out << interactions::sampleEps(rng(), E, 2., xs, cmb) / SI::eV << "\t";
      out << interactions::sampleEps(rng(), E, 2., xs, ebl) / SI::eV << "\t";
      out << "\n";
    }
  }
}
